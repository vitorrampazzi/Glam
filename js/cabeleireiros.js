const divCabeleireiros = document.querySelector('#tCabeleireiros');

cabeleireiros.forEach(cabeleireiro => {
    {

        divCabeleireiros.innerHTML +=  `    
        <div class="col-md-4 my-3">
            <div class="card bg-card p-4 rounded-4">
                <img src=".${cabeleireiro.foto}" class="card-img-top rounded-2 mx-auto"
                alt="barbeiro xpto">
                <div class = "card-body text-center text-light w-100">
                <h5 class="">${cabeleireiro.nome}</h5>
                <h6 class="mt-2 mb-4">${cabeleireiro.especialidade}</h6> 
...
document.addEventListener('click', function (e) {
    ...
    if (e.target.classList.contains('modal-item')) {
        ...
        document.querySelector('#precoss').textContent = e.target.getAttribute('data-preco')
        document.querySelector('#avaliacaoss').textContent = e.target.getAttribute('data-avaliacao')
